...
; ═══════════════════════════════════════════════════════════════════════════
; KRONOS THEME — the only edits to Tauri's stock template.
;
; This file is a copy of installer.nsi from tauri-cli 2.11.4 (the version in
; package.json). Everything outside this block is Tauri's and must be
; re-diffed against upstream when the CLI is upgraded, or a newer CLI's
; fixes silently stop applying to this installer. The block is fenced so the
; diff is obvious.
;
; What a theme CAN reach in NSIS's Modern UI: the welcome/finish page body,
; the header strip, the page titles and copy, and the images. What it
; cannot: the inner pages' dialog surface and the Next/Cancel buttons, which
; Windows draws itself in the system dialog colour. So the wizard is dark
